...
│   ├── ReelController.cs       ← Per-reel spin logic
│   └── UIAnimator.cs           ← Visual effects & animations
├── Prefabs/
│   ├── Reel.prefab             ← Single reel prefab
│   ├── BetMenuPanel.prefab     ← Bet selection popup
│   └── WinPopup.prefab         ← Win celebration popup
├── Animations/
│   └── ReelSpin.anim           ← (Optional) Animator override
├── UI/
│   ├── bg_gradient             ← Background texture
│   ├── slot-machine1–5         ← Machine sprite sheets
│   ├── slot-symbol1–4          ← Symbol sprites
│   ├── slot_machine_buttons    ← Button sprite sheets
│   └── Yes_No_Btn              ← Confirm dialog buttons
└── Sounds/
    ├── spin.wav
    ├── win.wav
    ├── jackpot.wav
